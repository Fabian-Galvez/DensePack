...
def main():
    ...
    # A leg with an API error has no price and no score, so the summary
    # leaves it out and says how many it left out.
    failed = [r for r in rows if r.get("money") is None]
    rows = [r for r in rows if r.get("money") is not None]
    if failed:
        print("\n%d leg(s) had an error and are not in the summary" % len(failed))
    ...
    if off and on:
        ...
        print("     scores %s" % dict(Counter(tuple(r.get("score") or ()) for r in on)))
        print("     arenas %d  sessions %d  first cache read %s"
              % (len({r["arena"] for r in rows}), len({r["session"] for r in rows}),
                 sorted({r.get("first_cache_read") for r in rows},
