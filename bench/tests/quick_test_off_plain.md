...
Run `cd <repo folder> && python bench/session_cost.py "$(python bench/own_transcript.py quick_test_off_plain.md)"`.
`own_transcript.py` returns the chat whose first prompt names this file, which
is this chat, whatever else is open.
Put its TOTAL row's Tokens and Dollars into the test 3 row of the results table in
