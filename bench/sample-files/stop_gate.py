...
from common import (disabled, emit, has_edited, read_event, read_leads,
                    read_totals, settings, stale_agents, tmp_dir,
                    transcript_path, user_stopped_agents)
...
# An agent Claude Code marked stoppedByUser never reaches SubagentStop, the
# stall scan leaves it out on purpose, and no notification is sent for it, so
# until 2 September 2026 the lead was never told. An interrupt of ONE lead
# tool call marks EVERY background agent that way at that same instant:
# three builders died at the second a user rejected one Bash call, and
# the lead reported them running for 42 minutes. Told once per agent,
# through the same told file a stall uses.
KILLED_REASON = (
    "STOP. The agent assigned to \"%s\" was stopped by a user interrupt "
    "before it reported. Every background agent stops the instant any lead "
    "tool call is interrupted, and no notification is sent for it. Do not "
    "say it is running. Check what it left on disk, and in this reply "
    "either relaunch it with the same brief or tell the user it died."
)
...
def main(event=None):
    ...
    # the session is on the event. guarded_main() below reads it once and
    # hands it in, so the tracker row it refreshes afterwards names the
    # same session this run gated.
    event = read_event() if event is None else event
    ...
    if not retried:
        ...
        if fresh:
            ...
            return 0
        # Agents a user interrupt killed, told once each, after any stall
        # because a stall in progress can still be nudged while this cannot.
        killed = [row for row in user_stopped_agents(session)
                  if row[0] not in seen]
        if killed:
            who, started = killed[0]
            seen.add(who)
            try:
                told.write_text(json.dumps(sorted(seen)), encoding="utf-8")
            except OSError:
                pass
            emit({"decision": "block",
                  "reason": KILLED_REASON % job_for_marker(session, started)})
            return 0
...
def track(event):
    ...
    totals file this project already keeps. This hook runs at the end of a
    turn, so it refreshes the figures and leaves the turn count to
    prompt_card.py, which runs on the message that opened the turn. It
    never raises: the row is a record for the dashboard, never part of the
