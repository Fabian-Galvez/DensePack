...
import gate_cost
from common import (disabled, emit, has_edited, is_subagent, read_event,
                    read_leads, settings, tmp_dir, user_said)
...
MESSAGE = (
    "DensePack note, not a block: this is hands-on call number %d in a "
    "row, Read, Grep, Glob or an inspection command, with no subagent "
    "spawned between them. Work a subagent can carry whole is cheaper "
    "there than in the lead's context. The call goes ahead. The user can "
    "say \"%s\" to silence this note, and /agentpack-off turns it off."
)
    ...
    try:
        ...
        emit({
            "hookSpecificOutput": {
                ...
                "additionalContext": MESSAGE % (count, ESCAPE),
            }
        })
