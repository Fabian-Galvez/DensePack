...
from common import (TRANSCRIPT_TAIL_BYTES, disabled, emit, read_event,
                    transcript_path)
...
def last_assistant_time(path):
    ...
    The transcript is read from the end, the last TRANSCRIPT_TAIL_BYTES
    only, the same tail common.last_touched_file() reads, because the last
    message is what matters and these files reach hundreds of megabytes.
    """
    import json
    path = Path(path)
    try:
