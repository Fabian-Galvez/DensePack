...
takes effect at the very next agent.
  ...
  python dpctl.py reader auto|fable|opus|sonnet
                                            ...
                                            One page size serves every reader
...
def caller_session():
    ...
    7ef6a4f5-b414-5bef-1a70-70f165b925d3, which is the id that project's
...
| One page for every model | Every report and brief is drawn at the one page size, whatever model wrote it or reads it. Not a setting |
...
def status_line():
    ...
    # The maxtier line that stood here went on 12 September 2026 with the
    # rule it described; one page size serves every reader.
    tier = ""
...
# READER_DATES, the day each reader's own size was measured, went on
# 12 September 2026 with the per-reader sizes.


def main(argv):
    # /setpack hands its words as ONE quoted argument since 12 September
    # 2026, so a stray ; or & the user typed after the command never reaches
    # the shell. They are split here and only these letters survive. The
    # colon and the backslash are in the set because "keep" takes a folder
    # and a Windows folder holds both. Audit of 12 September 2026.
    ...
    if verb == "status":
        ...
        if mode == "auto":
            print("DensePack reads the lead's own model off the first "
                  ...
                  "is drawn at one size, %d px, for every model. Right now: "
                  "%s." % (CODE_PX, resolved_reader(session)))
        else:
            print("DensePack reader set to %s, %s. Every image is drawn at "
                  "one size, %d px, for every model, so this changes the "
                  "name the plugin reports and no page. Set it back to auto "
