...
# Hedging. A word that softens a fact the writer already knows. The rule is
# "no hedging" in WRITING-RULES.md, added 9 September 2026 with the sentence
# shape table. A closed list, the same design as IDIOMS and LABEL_NOUNS. An
# open list would flag a sentence that really is stating a tolerance.
#
# "about" and "around" are left off on purpose. Both carry a real meaning in
# front of a number, "about 5 KB", and this plugin's own documents use them
# that way.
HEDGES = (
    "arguably", "i think", "i believe", "it seems", "kind of",
    "may or may not", "more or less", "perhaps", "possibly", "presumably",
    "probably", "seems to", "somewhat", "sort of", "we think",
)
HEDGE_RE = re.compile(
    r"\b(?:%s)\b" % "|".join(re.escape(h) for h in HEDGES), re.I)


def find_hedge(text):
    """Hedging words, as (kind, phrase) pairs."""
    return [("hedge", m.group(0)) for m in HEDGE_RE.finditer(_strip(text))]


# Phrasal verbs. A verb plus a particle whose meaning is not the two words
# put together. The rule is "no phrasal verbs" in WRITING-RULES.md. Each entry
# carries the plain verb, so the message names the replacement the way
# find_jargon does rather than only naming the fault.
#
# A closed list again. "set up" is here and "set" is not, because "set the
# value" is correct English and needs no replacement.
PHRASAL = {
    "kick off": "start",
    "kicks off": "starts",
    "spin up": "start",
...
PHRASAL_RE = re.compile(
    r"\b(?:%s)\b"
    % "|".join(re.escape(p) for p in sorted(PHRASAL, key=len, reverse=True)),
    re.I)


def find_phrasal(text):
    """Phrasal verbs, as (kind, phrase) pairs naming the plain verb."""
    hits = []
    for match in PHRASAL_RE.finditer(_strip(text)):
        plain = PHRASAL.get(match.group(0).lower(), "")
        hits.append(("phrasal", "%s (use %s)" % (match.group(0), plain)))
    return hits


# NO SEMICOLON CHECK, and this is why. WRITING-RULES.md bans the semicolon,
# so a check looks obvious. It was written on 9 September 2026 and cut the
# same hour. Dry run over the 58 markdown files here: 35 of them fired, and
# every slash command fired on its own shell line,
# "p=python; command -v python ...", which is a command and not prose.
# _strip removes fenced code blocks and does not remove a bare command line.
# This file's rule is that a pattern firing on good prose gets cut, so it was.
# A semicolon is one character and the writer can see it. Do not add this back
# without a way to tell a command line from a sentence.
...
SCOPES = {
    ...
    "passive": "write",
    "gerund": "write",
    # ADDED 9 September 2026 with the sentence shape table in
    # WRITING-RULES.md. A hedge and a phrasal verb both change what the
    # reader is told, so they block a chat reply as well as a file.
    "hedge": "both",
    "phrasal": "both",
}
...
def find(text, scope="write"):
    ...
    hits.extend(find_hedge(text))
    hits.extend(find_phrasal(text))
