...
from common import (MARKER, append_lifecycle, append_queue, code_size, disabled,
                    font_size, keep_copy, lead_model_name, pending_path, read_turn_fee,
...
def report_floor_chars(session_id=None):
    ...
    Since 7 September 2026 the floor is the lead session's own Read turn
    fee, common.read_turn_fee, the same number the saving test below
    applies, so the first pass and the second agree: on the hooks bench
    four agents were blocked for the marker line over a flat floor and
    their reports then shipped as text under the fee, one wasted turn each.
    ...
    whole extra turn for nothing: measured 6 September 2026 on five Fable
    readers whose one line replies ran 150 characters, 13,025 to 13,484
    tokens each, bench/session-2026-09-06/PAIR-2026-09-06.md. With the
    plugin stood down the same readers took three turns and the picture arm
    beat the text arm on cash.
    """
...
def main():
    ...
    # Since 7 September 2026 the first pass asks whether a pack pays in
    ...
    worth = report_pack_worth(len(content) / 2.40, lead_model_name(event.get("session_id")),
    ...
    if (not marker_found and dangling is None and not blocked_flag.exists()
        ...
        # the agent is never asked to write anything. 6 September 2026.
        ...
        queue_text_row(base, "under the saving threshold",
                       chars=len(content),
                       text_tokens=round(len(content) / 2.40))
        return 0
    ...
    try:
        # Real newlines since 12 September 2026: the pilcrow mark flatten()
        # used to join with was drawn as the word "[pilcrow]" at every line
        # end of every report.
        ...
        # Since 8 September 2026 a report draws through the code page
        # renderer, a design decision; the tags stay, so an id still travels
        # exactly in the sidecar.
        import codepack
    ...
    if legend_file:
        # The full path, never the bare name; the same fix brief_pack.py
        # carries, 2 September 2026, after agents searched the disk for a
        # legend named without its folder.
    ...
    # The Read that opens the image is a whole extra lead turn, not the 80-token
    # envelope an older comment here assumed. Forensics on the 2 September 2026
    # quick test measured the plugin-on lead spending 14,031 more thinking
    # tokens than the plugin-off lead across about seven extra turns, ~2,000 a
    # turn, because each image Read re-reads the growing context and opens a
    # fresh thinking block. A text report is read inline and pays none of that.
    # So a report only earns packing when its token saving clears one Read
    # turn; below that the image costs more than the text it replaces, which is
    # why the small file-reading reports made plugin-on cost more than off.

    # The fee is this session's own, from its context, since 7 September
    # 2026; the flat READ_TURN_TOKENS stays as the stub floor's yardstick.
    ...
    # size the report itself draws at. Fable read 5 of 5 blocks off the clean
    # pair in bench/abclean-2026-09-02.md on 2 September 2026, so the old rule
    # that code cannot be read from an image no longer holds for python. Every
    # other fence label still goes out as text only, and the .txt keeps every
    # block either way, so a reader that needs an exact byte still has one.
