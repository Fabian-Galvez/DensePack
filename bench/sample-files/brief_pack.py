...
POINTER = (
    ...
    "%s. The shared image named at your start explains the format.%s"
)
...
# Named after CODE_NOTE, on the same pointer, when python blocks drew banded.
CODE_IMAGE_NOTE = (
...
def main():
    ...
    # caller. LIVE FAILURE 1 September 2026: a claude-code-guide spawn
    # with no model field fell through to the caller chain, took the
    # Fable lead's 8 px, and its agent reported the brief unreadable.
    # An unidentified receiver gets text.
    if not model and (
    ...
    # The /maxpack rule that stood here, "an Opus lead may not brief a Fable
    # agent at 8 px unless maxtier is on", went on 12 September 2026. Every
    # size is CODE_PX, so px was never 8 and the rule never fired.
    ...
    if fences:
        ...
        # the subagent reads it off the picture. Fable read 5 of 5 blocks off
        # the clean pair in bench/abclean-2026-09-02.md on 2 September 2026.
        # Any other fence label still goes out as text only.
        import codepack
    ...
    try:
        ...
        # Since 8 September 2026 the brief draws through the code page
        # renderer, the page every reader read 5 of 5 all day: black
        # letters, blue digits, red marks, a key row, 756 or 840 px wide.
        # The prose renderer drew every letter in its own look-alike ink at
        # 8 px in a 330 px column, and a Sonnet reader could not read it.
        # The tags stay: a path or an id still travels exactly in the
        # sidecar, and the picture shows [#N] where it sits.
        import codepack
        from common import code_size
        # The reader name reaches the manifest row only; pack_code draws one
        # page for every reader. It used to be picked by comparing px with
        # the Sonnet size, which with one size always gave the same answer.
    ...
    if legend_file:
        # The full path, never the bare name. FIXED 2 September 2026: three
        # agents in one session each received "Tags: densepack-legend-<hash>
        # .txt" with no folder and each ran a find over the whole user
        # profile to locate it before it could read one exact path.
