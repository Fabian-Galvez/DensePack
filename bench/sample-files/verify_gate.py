...
already finds. A transcript that cannot be read is not
...
from common import (disabled, emit, is_subagent, line_pull, read_event, read_leads,
                    settings, sibling_image, user_said)
...
# A project's own source or test tree, a folder named src or tests anywhere
# in the path. Until 12 September 2026 the pattern also required a folder
# named Repos above it, the layout of one working tree, and matched nothing
# in a project that lives in its own repository.
...
SRC_TREE = re.compile(r"[/\\](?:src|tests)(?:[/\\]|$)")


def in_project(path):
    """True when the path sits under the project the session works in. The
    pattern above matches a src or tests folder anywhere on the disk, an
    installed library's included, and a Read outside the project is not a
    source read of this project. Audit of 12 September 2026."""
    root = os.environ.get("CLAUDE_PROJECT_DIR") or os.getcwd()
    root_norm = str(root).replace("\\", "/").rstrip("/").lower() + "/"
    return str(path).replace("\\", "/").lower().startswith(root_norm)
...
def main():
    ...
    try:
        ...
        # green number passes before the sidecar redirect and before the
        # source rule: it is the one cheap way to check a value the image
        # did not give clean, and it must never come back as the image.
        ...
        if is_image(path):
            ...
            source_read = False
        elif SRC_TREE.search(path) is not None and in_project(path):
            message = SRC_MESSAGE
            source_read = True
        else:
            return 0

        if user_said(event.get("session_id"), ESCAPE):
            return 0
