"""Notes a spawn that carries less work than it costs. The spawn goes ahead.
...
writes holding the words "floor override approved" draws no note at all.
...
into its next brief and spawn under the floor unchecked. SEEN LIVE 31 August
2026: this gate refused a subagent's spawn and gave it the override in the
same sentence. The note therefore never carries the phrase, and the phrase in
a subagent's brief does not clear the gate. common.is_subagent() tells the
two apart.
...
import gate_cost
from common import disabled, emit, is_subagent, read_event, tmp_dir
...
MESSAGE = (
    "DensePack note, not a block: this brief is %d characters, under the "
    "%d character floor, and a brief that short has carried fewer than "
    "three commands 30 times out of 44 in this project's own history. A "
    "spawn pays for itself when it batches several commands, so widen the "
    "brief if more of this work can travel with it. The spawn goes ahead."
)
    ...
    try:
        ...
        if should_stop(brief, honour_override=not spawner_is_agent):
            if batch is not None and batch.exists():
                return 0
            # MECHANISM 2, the intervention cap. Two notes in one session
