...
here, in common.AGENT_MODEL_FILE, keyed on actor_key(event), this one
agent's own id: see that function's own docstring for why neither
session_id nor the transcript stem can serve as the key on THIS event.
"""
...
from common import (agent_meta_model, agent_type_model, append_lifecycle,
                    ...
                    actor_key, vault_dir, DEFAULT_CARD,
...
POINTER = (
    ...
    "chat message must follow, and the format every report "
    "carries."
)
...
# A leaf worker reads this text instead of the role and rules images.
# Measured 2 September 2026: pointing every spawn at workerrules-1.png cost
# 627 image tokens plus a Read turn, and made a fact-gathering worker spend
# turns conforming to writing and code rules its report does not need. The
# subagent side then ran 5.4x the plugin-off cost on a small task. This text
# carries all the packer needs, a final-message report, summary first, one
# finding per line, in literal sentences, and it names the scope rule the
# 27 August 2026 note added. A worker that itself orchestrates is rare and
# still gets the format it needs from this same text.
WORKER_INSTRUCTION = (
    "OUTPUT FORMAT REQUIREMENT from the user's DensePack plugin. Your final "
    "chat message IS your report; never write your report to a file. Start "
    "with a summary of five lines or fewer, then the full report, one "
    "finding per line. Write plain literal sentences. Name the files you "
    "changed and the checks you ran; do not name anything you left alone."
)
...
def main():
    ...
    # event itself never carries. Keyed on actor_key(event), this one
    # agent's own id, never on event["session_id"] and never on the
    # transcript stem: this event's transcript_path is the LEAD's as often
    # as this agent's own, so both of those name the spawning session, the
    # same fact "spawned_by" above already relies on.
    real_model = agent_meta_model(agent_id)
    if real_model:
        reader = reader_key_for_model(real_model)
        if reader:
            record_agent_model(actor_key(event), reader)
    ...
    # A leaf worker gets the report format as text, not the role and rules
    # images. See WORKER_INSTRUCTION's own note for the 2 September 2026
    # measurement that removed the image load. worker_folder() and
    # card_folder() are kept for a later opt-in path that a sub-orchestrator
    # could use, and they cost nothing while no caller reaches them.
