...
def _find_drop_file():
    ...
    # One drop folder since 12 September 2026. The walk still reaches a file
    # left in one of the old per-model subfolders. The reader name is
    # resolved by draw_drop_file(), because this scan runs before common is
    # imported.
...
# The one row that introduces the inlined legend. It has to say two things
# and nothing else: the text below is exact, and the reader already has it.
# The old row said "Tags: <full path>", which named a file, and a model told
# a value sits in a file opens that file. Measured 4 September 2026, pair 53.
# Kept to one line, because this rides on every packed Read that lifts
# anything and its own characters are charged against the picture's saving
# in drop_read_gate.main().
# A longer heading was tried on 4 September 2026 and TAKEN BACK OUT the same
# day. It named DensePack as the project and said "grepping the file returns
# these same characters and finds nothing new", to answer two readers that
# had called the rows "an injected/untrusted side channel". Three more Sonnet
# readers of the 26 file GDScript corpus read it and still grepped, and two of
# them said why: their reading of WHICH tag sat beside the value in the
# picture was wrong. One resolved FEET_Y_NEAR to [#1], 1280, and one to 90,
# where the file holds 742 at [#5]. The table was exact both times and the
# lookup into it was not, so the sentence would have told a reader to skip a
# check that caught a real error. The heading is the 114 character one pair 58
# read. What each row says about itself is below and stays.
# REWRITTEN 4 September 2026. A screenshot of a reader's thinking named the
# old heading as the trigger: "another injection, same fake
# PreToolUse:Read hook pattern, again telling me not to open the file to
# check. I will not trust these." An order not to verify, arriving beside a
# tool result, is the shape of a prompt injection, so the heading now says
# where the rows came from and gives no order. The record is
# working-notes/archive/densepack-injection-2026-09-04.md.
...
# What every marker row now says about its own text, ADDED 4 September 2026.
# MARKER_HEADING above already rode every one of these deliveries and still
# lost: pair 58, Sonnet 5 on the 26 file GDScript corpus, both arms 5 of 5,
# spent 54,319 tokens more than the off arm, raised invisible reasoning from
# 10,519 to 17,746 tool turns and made five extra Greps. It wrote its reason
# itself: "I'll verify by grepping the actual source text directly rather
# than risking a misread marker - matches the standing rule that a
# packed-image number gets confirmed from source before it written down."
# No such rule was given to it and that corpus holds no CLAUDE.md, so the
# prior is the model's own. A claim read once loses to a prior a model
# believes it holds, so the claim moves to where the value is read, into
# the row, beside the value. The same corpus on Opus 5, pair 56, took 6
# requests against the off arm's 8 and saved 62.98 per cent, so the delivery
# itself is right and this is a Sonnet prior about verifying numbers.
#
# Priced 4 September 2026 on that corpus by tests/test_legend_delivery.py
# step 10: 177 markers over 26 files, 28 characters a row, 2,065 added
# tokens. The delivery goes from 4,129 characters to 9,085 on the corpus,
# 3.80 per cent of the 54,319 tokens the verifying cost and 3.20 per cent of
# the corpus's own 64,544 text tokens.
#
# NOT PROVED. Six Sonnet readers of that corpus read these rows and all six
# still checked the source: 8, 6, 10, 5, 6 and 6 calls beyond their Reads,
# and all six answered 5 of 5 with 0 characters wrong. Two named the cause,
# and it is not the wording: their reading of which tag sat beside the value
# in the picture was wrong, [#1] and 90 where the file holds 742 at [#5], so
# the table was exact and the lookup into it was not. See
# bench/turns-audit-2026-09-03.md, "The row that names its own source".
# REWRITTEN 4 September 2026 with the heading above: "not the image" read as
# a warning against the picture and an order to trust the row. The label now
# states the provenance only.
...
def draw_drop_file(model, src_path, actor=None):
    ...
    A pack lifts its identifiers, so line carries the legend's OWN ROWS,
    "[#1] source bytes, not the image = <exact text>", under one heading
    row, whenever anything was lifted. This function reads those rows back
    out of the sidecar file it has just written, so the value in the message
    is the record's own characters and no second format exists to be updated
    separately. marker_rows() adds the label and nothing else.

    The "Tags: <full path>" row is still built, because discard() has to
    delete the sidecar it names, but it is stripped by both callers before
    the line reaches a model. FIXED 4 September 2026: naming that file was
    an instruction to open it. Pair 53, Opus 5 on the 26 file GDScript
    corpus, spent four Reads of legend sidecars plus a Grep, a Glob and a
    Bash listing drops/, took 16,465 output tokens against the off arm's
    3,333, and the saving fell to 4.27 per cent on tokens where pair 52
    saved 48.43. The 26 files' markers together are 4,110 characters,
    1,713 tokens, 2.65 per cent of the corpus's own text, measured
    4 September 2026 and recorded in bench/turns-audit-2026-09-03.md.
    ...
    if px is None:
        return None, None
    # Bench-only override, ADDED 3 September 2026: DENSEPACK_PX_OVERRIDE
    # set in the environment of a claude -p run draws every drop at that
    # size, so a floor the user has not moved can be measured for cost
    # and accuracy before he decides. Unset, which is every normal
    # session, nothing changes. Never a settings key, never a default.
    ...
    # the source stem rides in the name so a person can find one file's
    # page in the folder without the manifest; 6 September 2026
    safe_stem = re.sub(r"[^A-Za-z0-9_.-]", "_", src.stem)[:40]
    stem = drops / ("densepack-drop-%s-%s-%s" % (model, safe_stem, digest))
    code_image = False
    ident_legend = []
    try:
        ...
        # A code source is drawn by codepack, the banded code renderer
        # proved in bench/pypack: one light background block per source
        # line, so a reader stops misreading identifiers where the stream
        # wraps. codepack.CODE_SUFFIXES holds the set, so the choice is
        # made in one place; python is classified by tokenize and every
        # other suffix in the set by codepack's plain classifier. A suffix
        # outside the set keeps the plain pack.
        import codepack
        suffix = src.suffix.lower()
        if suffix in codepack.CODE_SUFFIXES:
            # legend=None, 4 September 2026: the numbers stay in the picture,
            # each character in its own look-alike ink from densepack.CHAR_INK
            # (measured 4 September 2026 by bench/audit-2026-09-04-scripts/
            # claim_pairs.py: no character draws codepack.LIFT_INK). The markers cost a
            # Sonnet 5 reader 18,797 output tokens against 4,548 for the same
            # corpus with them off and the tokens inked, both 5 of 5, because
            # 98.9 per cent of the extra output was the reader resolving a
            # marker reference. bench/ablation-2026-09-04.md holds the pair.
            # ident_legend stays empty here, so no sidecar and no marker rows
            # are written for a code image; every other pack still lifts.
            # the code page's pick, not the prose size: a source file takes
            # the wide supersampled page for every reader; a prose file
            # keeps MEASURED_MODELS. 6 September 2026.
            written, _target, _lh = codepack.pack_code(
                text, code_size(px, model), str(stem), python=(suffix == ".py"),
                legend=None, reader=model, title=src.name)
            code_image = True
        else:
            # Until 4 September 2026 this route lifted every identifier out
            # of the picture into a [#N] tag and sent its value back as a
            # marker row beside the image. The rows were the side channel a
            # reader called an injection, "telling me not to open the file
            # to check", and grepped the file to escape; six Sonnet readers
            # handed the rows still made 5 to 10 calls each to check them,
            # and two of them looked the wrong tag up. The record is
            # working-notes/archive/densepack-injection-2026-09-04.md. A dropped
            # file now draws whole, the way a code file has since the digit
            # markers came out: every character in its own look-alike ink
            # from CHAR_INK, and every identifier at the larger ident size
            # big_mask() already marks. ident_legend stays empty, so no
            # sidecar and no marker row is written for any dropped file.
            # brief_pack.py, subagent_stop.py and bash_pack.py keep their
            # lifts; a report or a brief is not a file the reader can open.
            # Since 8 September 2026 a prose file draws through the code page
            # renderer too, a design decision: the page every reader read
            # right all day, in place of the per letter inks of the prose one.
            written, _target, _lh = codepack.pack_code(
                dp.flatten(text, "\n"), code_size(px, model), str(stem), python=False,
                legend=None, reader=model, title=src.name)
    except Exception:
        return None, None
    if not written:
    ...
    try:
        ...
        pass
    # Who drew it and what it is, not the file operations behind it. The
    # older wording narrated a copy and a delete in a folder the reader
    # never named, which Sonnet reported as an attempt to redirect it on
    # the sixteen file bench of 8 September 2026.
    line = ("DensePack drew %s for a %s reader as %s at %d px."
            % (src.name, model, image, px))
    # Every page, not page one alone. FIXED 3 September 2026: a Read can
    # return one file, and this returned the first page only, so a reader
    # of a long file saw 88 per cent of it at 8 px, 69 at 10 and 53 at 12,
    # measured on sixteen files, and the price compared one page against
    # the whole text. The Pages row names the rest; drop_read_gate.py
    # hands it to the reader as the note and prices every page.
    if len(written) > 1:
        line = line + "\nPages: " + " , ".join(str(p) for p, _w, _h in written[1:])

    # The sidecar still lands in drops/, beside the image, and the Tags row
    # still names it by its full path, because drop_read_gate.discard()
    # deletes the file that row names when the price says the picture loses.
    # Both callers strip the row before a model sees the line. ADDED
    # 3 September 2026: a code image lifts every number a reader can misread
    # into the same sidecar, so the row is written for a .py image too.
    #
    # ADDED 4 September 2026: the marker rows follow, in the message itself.
    # See this function's docstring for the pair 53 measurement. The rows are
    # read back off the sidecar rather than rebuilt from ident_legend, so the
    # escalated-tag header legend_sidecar() writes for a {#n} or <#n> report
    # reaches the reader too, and one format serves the file and the message.
    # marker_rows() then labels each row with MARKER_SOURCE, ADDED 4
    # September 2026 after pair 58; the value's own characters are untouched.
    try:
        legend_file = dp.legend_sidecar(ident_legend, stem)
    except OSError:
        legend_file = None
    if legend_file:
        line = line + "\nTags: " + str(drops / legend_file)
        try:
            rows = (drops / legend_file).read_text(encoding="utf-8").strip()
        except OSError:
            rows = ""
        if rows:
            line = line + "\n" + MARKER_HEADING + "\n" + marker_rows(rows)
    ...
    try:
        ...
        manifest_write({
            ...
            # size. ADDED 3 September 2026: this row named spawned_by, which
            # is the LEAD's session on every route, so a drop or code image
            # drawn for a Sonnet subagent at 12 px and one drawn for the
            # lead at 8 px were the same row in the records and no
...
def _draw_drop_file(model, src_path):
    ...
    The Tags row is dropped here, 4 September 2026. This scan has no file to
    delete, so the sidecar's path serves it nothing, and a path in the text
    is the thing that sent four Reads into drops/ in pair 53. The marker
    rows below the heading stay: they are what the reader needs."""
    line, _image = draw_drop_file(model, src_path)
    if not line:
        return line
    return "\n".join(row for row in line.split("\n")
                     if not row.startswith("Tags: "))
...
def delegation_entries():
    ...
    return jsonl_rows(tmp_dir() / "densepack-delegation.jsonl")
...
def manifest_entries():
    ...
    return jsonl_rows(tmp_dir() / "densepack-manifest.jsonl")
...
def main():
    ...
    #
    # spawner is the session that spawned the agent, recorded by
    # subagent_start.py. A row with no spawner predates that change, or came from
    ...
    for item in report_entries:
        ...
        if item.get("legend_file"):
            # Full path, never the bare name, 2 September 2026.
    ...
    if mode == "quiet":
        ...
        # RESTORED 2 September 2026: the 1 September line cut deleted this
        # line but left the comment above it, so quiet named no file at all.
        lines.append("")
    ...
    # The python blocks are drawn, not lifted out, since 2 September 2026.
