...
def targets_full_rules(path):
    """True for a Markdown document or any file named README*. False for a
    file under .claude, a tmp, sandbox or scratch folder, or node_modules,
    which pass at no cost.

    Until 12 September 2026 a Markdown file counted only with a folder named
    Repos above it, which was the layout of one working tree,
    a Repos folder over each project. A project in its own repository has no such
    folder, so the gate watched nothing but README there.
    """
    norm = str(path).replace("\\", "/")
    # A relative path is relative to the session's folder. Without this a
    # Write to "notes.md" passed the project test below. Audit of
    # 12 September 2026.
    if not os.path.isabs(norm):
        norm = os.path.join(os.getcwd(), norm).replace("\\", "/")
    parts = [p for p in norm.split("/") if p]
    if not parts:
        return False
    name = parts[-1]
    folders = [p.lower() for p in parts[:-1]]
    if any(f in (".claude", "tmp", "node_modules") for f in folders):
        return False
    if name.startswith("README"):
        return True
    if not name.lower().endswith((".md", ".markdown")):
        return False
    # A Markdown file inside the project the session works in. A note in
    # some other folder on the disk is not a document of this project.
    root = os.environ.get("CLAUDE_PROJECT_DIR") or os.getcwd()
    root_norm = str(root).replace("\\", "/").rstrip("/").lower() + "/"
    return norm.lower().startswith(root_norm)
...
def style_deny(event, path):
    ...
    targets_full_rules(). Every one of literal_check's seven checks applies
    ...
    if settings().get("stylecard", "off") != "on":
        return None
    if not targets_full_rules(path):
        return None
...
def rules_image(event):
    ...
    if reader in MEASURED_MODELS:
        # One rules image for every reader since 12 September 2026; the
        # per-model folders drew it at two sizes.
        return vault_dir() / "instructions" / "allrules-1.png"
