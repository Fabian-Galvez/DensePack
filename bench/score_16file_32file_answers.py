...
for i, parts in enumerate(KEY, 1):
    ...
    if WHICH in ("py", "py32"):
        ...
        # a digit or a dot on either side also fails. A dot that ends the
        # sentence is not part of a number, so "6." scores. Case does not
        # count, the same as the test above.
        # Any listed form counts, so write_gate scores beside write_gate.py.
        ok = any(re.search(r"(?<![\w.])%s(?!\w|\.\w)" % re.escape(p), ans, re.I)
                 for p in parts)
    right += ok
