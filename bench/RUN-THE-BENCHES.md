...
It explains the benches in plain words. You can check what your AI does.

## Before you start

1. Install DensePack with the steps for your system in [INSTALL.md](../INSTALL.md).
2. When Claude Code asks for the install scope, choose one of the scopes.
   Local scope does not bench better and does not make removal easier.
...
7. Say yes when Claude asks to start.
   ...
   It continues to the next bench only when the image arm of the last pair scored 5 of 5.
...
| What each bench converts | All files, cold. Each arm starts in an empty folder |
...
Those 32 scripts are copies of scripts from a previous version of the DensePack plugin. 16 of them are not in `plugin/scripts/` now.
The rebuild bench has no off arm because an agent that reads a file as text copies the text with no error.

## 3. What your AI does, step by step

1. It tells you which benches will run and in what order.
2. It tells you that each pair uses your Claude plan or your API credits.
3. It waits for you to say yes.
4. It runs `python bench/check_bench_setup.py`. That command must print PASS. On anything else it stops and shows you the output.
5. For each model it runs the single file bench first, then the 16-file bench, then the 32-file bench.
   A request for the 32-file bench runs all three. A request for the 16-file bench runs the first two.
   A single file pair costs a few cents. A fault on your machine costs little.
   It continues to the next bench only when the image arm of the last pair scored 5 of 5.
...
10. On PARTIAL it tells you why and continues to the next pair.
11. On FAILS it tells you which questions the image arm missed, or how much more it cost.
    Opus and Fable answer all questions and cost less. On FAILS it stops and asks whether to investigate.
12. When a Sonnet pair fails, or saves less than the lowest Sonnet pair for that bench in BENCHMARKS.md, it gives you choices.
    You can type `/max-off`. It makes Sonnet read text instead of images, like Haiku. Fable and Opus still receive images.
    You can run the pair again because Sonnet sometimes reads all images and then saves less.
    When the image arm scored 5 of 5, a third choice is "Savings are savings: MaxPack it!" Sonnet keeps images. That is the default. The benches continue.
...
All commands run from the folder that holds README.md.
Put `DENSEPACK_BENCH_EFFORT=high` in front of each bench command in the Bash tool. The two arms then think at the same effort.
In PowerShell, run `$env:DENSEPACK_BENCH_EFFORT = "high"` once before the commands.
Give each run a new name, such as `sonnet-single-1`.
...
The on arm turns `/maxpack` on inside its own bench folder. Sonnet gets images on that arm.
...
| --- | --- |
| The two arms finished with no error and no refused tool call | A broken run measures nothing |
...
| The two arms started with the same first cache read | A warmer start makes the billed price of one arm lower by chance |

| Verdict | What it says about the pair |
| --- | --- |
| COUNTS | The pair counts at the billed price and at the cold price |
| PARTIAL | The first cache reads differ. The pair counts at the cold price only |
| FAILS | The on arm answered fewer questions, or it cost as much or more |
| DISCARD | An arm had an error, or the images went to the wrong arm. The pair measured nothing |
...
The single file scorer ignores letter case. `densePack` scores the same as `densepack`.
The checker prints the turns and the Reads of each arm. The turns and the Reads never decide the verdict.
The price counts each turn an arm takes.
The on arm can use more turns or open only the images it needs and still cost less.
In pair `sonnet-thirtytwo-1` the on arm took 6 turns and the off arm took 4.
The two arms answered 5 of 5. The on arm cost 60.5% less at the cold price.

In pair `sonnet-sixteen-1` the off arm started with 0 cached tokens and the on arm with 1,964.
The cold price charges those 1,964 tokens as new tokens. The cold price removes the difference.
...
The token prices are in `RATES` in bench/session_cost.py and in `rates()` in bench/price_bench_transcript.py.
...
Each transcript line records the two kinds in `cache_creation`: `ephemeral_1h_input_tokens` and `ephemeral_5m_input_tokens`.
...
Your AI recomputes at least one pair itself, from the transcript tokens and the published prices.
`cli_cost` is Claude Code's own figure. It can differ from that sum by about a tenth of a cent.
If the numbers differ from the checker's, your AI shows you its numbers and the checker's numbers.
...
- The repository folder from the start of this file. The benches load that folder's own plugin. An installed copy changes no result.
- Plan usage or API credits. Each arm is a real Claude Code conversation.
- Memory. A 32-file on arm used about 5 GB while it converted 32 files.

The two arms switch an installed DensePack off with bench/settings/off-plugin.json.
The on arm loads this download's plugin folder with `--plugin-dir`.
Each arm runs in its own empty folder in `DensePack-arenas` in your home folder.
...
| Each arm, one row a line | bench/rows/all-arms-rows.jsonl |
