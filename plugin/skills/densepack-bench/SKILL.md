...
Tell the user what will run and what it costs, and wait for a yes before any bench command.
...
Say so in the plan you show the user.
Go on to the next bench only when the image arm of the last pair scored 5 of 5.
When a pair FAILS, say plainly that it failed and which questions the image arm missed.
DensePack normally answers every question and costs less, so ask the user whether to investigate.
...
One: type `/max-off`, so Sonnet reads text and DensePack stays on for every other model.
Two: run the pair again, because Sonnet sometimes reads every image and saves less.
Three, only when the image arm scored 5 of 5: "Savings are savings: MaxPack it!" Sonnet keeps images, which is the default, and the benches continue.
