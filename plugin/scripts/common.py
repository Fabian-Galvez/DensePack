...
def settings(session=None):
    ...
    if path.is_file():
        ...
        if not isinstance(data, dict):
            data = {}
        # This file is in the project, so a cloned project can commit one. A
        # committed vault_mb of 0 makes the plugin delete its own kept copies
        # in that project, and "quiet" hides the receipt tables. Both settings
        # act only inside the project's .claude folder. The file has no seal,
        # because a seal makes every hand-written settings file inert.
