...
WHY IT EXISTS. Claude Code sends the whole
...
THE CONDITIONS. Ten of them. Each is checked in its own `if` and never joined
into one expression, because a gate whose terms are joined with `and` can lose
a middle term and read as false. A gate here means a
...
# everything said so far. Measured over 470 turns of one session: the mean
# was 220,917 tokens and the median 220,261. The mean is
...
def _pending_ids(paths):
    ...
    of only counting them. A row with no id field, or a
...
def waiting():
    ...
    # Every image the plugin packs sits under the project's .claude folder. A
    # row naming any other path came from a planted file, and the composite
    # would show that image to the lead.
...
def main():
    ...
    try:
        ...
        # A pointer older than the sweep: the tmp copy is gone, and the copy
        # keep_copy() put under this conversation's vault folder is not. The
        # Read is pointed at that copy, so a session open past 24 hours can
        # still open every picture it was handed.
        pending = waiting()
        ...
        # lead a second time, which happens when reports finish every few
        ...
        #    content with nothing saying so. Path is the module import; a
        #    local import here would make Path a local for the whole function
        #    and break the vault fallback above.
        ...
        #    turns it removes.
        #
        #    Comparing patches alone refuses every composite: three real
        #    images stack to 494 patches against 474 for the three apart.
        #    Stacking pages of the same width sums their rows and
