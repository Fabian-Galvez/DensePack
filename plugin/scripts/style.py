...
_CARD_OPENING = (
    ...
    # MEASURED 16 September 2026. Edit accepts a file that arrived as an
    # image, because the agent gives Edit the exact text it read off the
    # image. Write does not: Write replaces the whole file, so Claude Code
    # makes the agent read the text first, and a Read of that file gives the
    # image again. Without this line an agent that wants to change a file
    # loses turns finding that out.
    "To change a file that arrived as a picture, use the Edit tool with the "
    "exact text you read off the picture. Write refuses that file, because "
    "Write needs a text read and a Read gives the picture again.")
...
_CARD_CODE_SCHEME = (
    ...
    "are black, digits blue, the colon blue, the semicolon teal, the comma "
