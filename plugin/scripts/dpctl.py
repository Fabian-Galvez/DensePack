...
| Report packing | Each finished subagent's report is converted to an image when the image measures cheaper than the text |
| Brief packing | Each brief over the threshold is converted to an image before the subagent starts, at the one text size |
| One text size | Each report and brief converts at one text size for all models. Each image is 756 or 784 px wide. A longer text makes a taller image or more images. Not a setting |
| Refuse when worse | DensePack deletes an image that costs more than the words and sends the words |
...
| The delivery rule | Each subagent is told how to return its report |
| The manifest | Each agent that finishes gets a row, packed or not |
| The vault | DensePack copies each converted image and its text to .claude/densepack-vault, one folder a conversation. The copies stay when .claude/tmp is cleaned. When the vault is over 200 MB, DensePack deletes the oldest folders |
| The spawn log | Each agent SPAWNED gets a row too, before it finishes, holding the model it ran on. Recording only |
| The Edit check | DensePack stops an Edit whose old_string is not in the file, or is there more than once, before the Edit runs. The message quotes the file's own lines with all spaces and tabs visible. Send the Edit again with that text |
| Instruction files as images | At session start, the folder's CLAUDE.md, .claude/CLAUDE.md and CLAUDE.local.md, your ~/.claude/CLAUDE.md and the project's MEMORY.md each become a short pointer, the images and <name>.densepack.bak with the original text. A file converts only when its images and pointer cost less than its text. A short file stays text. It applies from the next session because Claude Code loads these files before DensePack runs. /dense-remove puts the originals back |
...
| /dense-remove | Puts each converted CLAUDE.md, CLAUDE.local.md and MEMORY.md back to its original text from its .densepack.bak, removes CLAUDE_CODE_THRIFTY_SONIC and deletes all DensePack files that /plugin uninstall leaves behind. Run it before the uninstall | Sets nothing |
...
The reader is the one setting that belongs to a single conversation. Run
dpctl.py reader inside the conversation it is for. /setpack keep both sets
both images and reports. dpctl keep images or keep reports sets one, and
takes a folder after it."""
    ...
    if verb == "status":
        ...
        if mode == "auto":
            ...
            print("DensePack reader set to %s, %s. Every image converts at "
                  "one size, %d px, for every model. This changes the "
