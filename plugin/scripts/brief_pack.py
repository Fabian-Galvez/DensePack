...
Three things must hold for this file to work, and each one does:
...
Which size, and when nothing happens at all:
  ...
  model named               the reader size for that model.
  ...
  receiver unidentified     NOTHING HAPPENS. An unreadable brief costs the
                            whole task, which is a far worse trade than the
                            tokens it would have saved.
...
# counted character for character instead of estimated. A fixed estimate
# undercounts it and lets a brief pack at a real loss while the receipt
# reports a saving.
#
...
# image. This route charges nothing for it.
# The agent reads the brief image on its own first turn, which costs a turn
# that an unpacked brief in the prompt never pays.
# The floor here is BRIEF_FLOOR alone, and a brief packs from 1,000
# characters up, measured by driving this hook.
# Whether charging the fee here earns money is NOT measured.
# Charging it would raise this floor by more than ten times, so it is the
# author's call and no agent may make it.
...
# match what SubagentStop reports, because a spawn row pairs to a finished
# run by type.
...
# The pointer carries only the paths: the color code and the reading rule
# live in shared.txt, drawn once at SessionStart and read by every agent
# before its first action.
POINTER = (
    "Your brief is the condensed image at %s. Read it with the Read tool "
    "and treat its text as your instructions, in full. Its exact text is "
    "%s.%s"
)
...
# Follows CODE_NOTE on the same pointer when python blocks draw banded.
CODE_IMAGE_NOTE = (
    " The python blocks are drawn banded, in order, in %s ; read those pages "
...
def main():
    ...
    # record is written here where both paths pass. A failure here must never
    # cost the spawn, so it is wrapped on its own and nothing below depends
    # on it.
    try:
    ...
    # miss most of them. The record names the model each agent runs on. A
    # failure here must never cost the spawn, so it is wrapped on its own and
    ...
    # A custom agent type with no frontmatter file can pin a model this
    # resolver cannot see. Only the default type provably inherits the
    # caller, so an unidentified receiver gets text.
    if not model and (
    ...
    if model:
        ...
        # A subagent spawning a further subagent reaches this branch.
        # font_size() would answer with resolved_reader()'s cached top-level
        # lead size, which is not the caller's model. event_reader() reads
        # the model of the agent making THIS call, from its own transcript;
        # see its docstring in common.py. reader_override() answers when the
        # session pinned a reader. Neither found means the receiver cannot be
        # identified, and an unidentified receiver gets text:
        # resolved_reader()'s UNKNOWN_READER fallback is a guess and must
        # never stand in for a measurement here.
        ...
        # densepack-session.json. A session named in that map is resolved
        # right here and never reaches the "map exists, so text" step below.
        ...
        if own_reader:
            ...
            # The reader pin sets what the LEAD reads and says nothing about
            # a spawn's model. An unset spawn inherits the CALLER's model, so
            # the session's own lead model answers next, and the flat pin
            # answers only as the last resort, and only in a project with no
            ...
            # in a project that has never pinned a per-session reader: there
            # the flat pin IS the project's one declared lead identity.
            # resolved_reader() is not used here because its UNKNOWN
            # fallback is a guess.
            declared = str(settings().get("reader", "")).lower()
            px = READER_SIZES.get(declared)
        else:
            # Once densepack-session.json exists anywhere in the project,
            # the per-session map is the scoped source of truth for
            # "reader", and a flat settings value is residue from whichever
            # window pinned it last. A caller not in the map and not named
            # by event_reader() or lead_model_name() above is unresolved,
            # and gets text.
            px = None
    if px is None:
        return passthrough()
    # The receiver is the first name the chain above matched. Sonnet gets
    # an image brief. Type /max-off to send it text.
    ...
    # Every size is CODE_PX, one image size for every reader.
    ...
    # Nothing can be sealed without the key, and a Read of the brief's text
    # would then never be swapped for its image. The brief goes as text.
    ...
    # The stamp names three files. The session id is the SAME for every agent
    # in a session, and parallel spawns are the normal case, so the session
    # id and the clock alone can give two spawns one stamp and let the second
    # overwrite the first's image. The process id separates concurrent hooks,
    # and the first free number separates two hooks that share both.
    ...
    if fences:
        ...
        # A python block draws banded, at the size this brief draws at, and
        # the subagent reads it off the picture. Any other fence label goes
        # out as text only.
        import codepack
    ...
    # newline="" keeps the brief's own line endings, so this file is the
    ...
    try:
        ...
        # use exactly. The brief keeps real newlines, because flatten()'s
        # pilcrow mark would draw as the word "[pilcrow]" at every line end.
        packed_text, ident_legend = dp.lift_identifiers(dp.flatten(content, "\n"))
        # The brief draws through the code renderer: black letters, blue
        # digits, red marks and a key row. The tags stay: a path or an id
        # travels exactly in the sidecar, and the picture shows [#N] where
        # it sits.
        import codepack
        from common import code_size
        # The reader name reaches the manifest row only; pack_code draws one
        # image for every reader.
    ...
    if len(paths) > 1:
        note = (" The brief continues in %s , in order.%s"
                % (" , ".join(paths[1:]), note))
    pointer = POINTER % (paths[0], source, note)
    # The legend goes to a sidecar file, the same as the report side: the
    # pointer IS the replacement prompt, so it gains one short tag line naming
    # the file, and the agent opens the legend file only when it needs one
    # value, rather than every value being retyped into every brief.
    legend_file = dp.legend_sidecar(ident_legend, stem)
    if legend_file:
        # The full path, never the bare name: an agent given only the file
        # name searches the whole disk for it.
        pointer = pointer + "\nTags: " + str(tmp_dir() / legend_file)

    # Sealed, so a Read of the brief's text is swapped for the image beside it
    # only where this machine's plugin wrote the pair.
    # One seal over the whole pack, so every image a reader is handed is one
    # this machine's plugin wrote.
    ...
    #   brief would
    # The Read calls that open the image and the code file are NOT charged
    # here. See the OPEN note at the head of this file.
    ...
    # The queue row above feeds the receipt table only, so the brief also
    # writes a manifest row, the same shape bash packs and agent reports
    # write. Its chars, text_tokens and image_tokens then count toward the
    # Without DensePack column in tools/live_dashboard.py like every other
    # packed kind.
    manifest_write({
        "packed": True,
...
def guarded():
    ...
    Only exit 2 blocks a PreToolUse call, so an unhandled error still lets
    the spawn go ahead with the original brief, but prints a full Python
    traceback on stderr for a saving that is optional by nature.
